...
public abstract class BaseInteractSystem
{
    ...
    protected IInteract executer;           // 상호작용을 시작하는 트랜스폼
    protected List<IInteract> receivers;    // 상호작용을 당하는 트랜스폼

    protected event Action StartAct;
    protected event Action EndAct;

    public BaseInteractSystem(InteractionDataSO data, IInteract executer, params IInteract[] receivers)
    {
        Init(data, executer, receivers);
    }

    // 초기화
    public virtual void Init(InteractionDataSO data, IInteract executer, params IInteract[] receivers)
    {
